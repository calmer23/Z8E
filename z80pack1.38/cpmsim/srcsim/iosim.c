...
#define CLMAUX		/* CLM override of AUX (RDR: and PUN:) usage */

#ifdef PIPES
static int auxin;		/* fd for pipe "auxin" */
static int auxout;		/* fd for pipe "auxout" */
#ifdef CLMAUX
static int aux_in_eof = 0xff;	/* status of pipe "auxin" (<>0 means EOF) */
#else  /* CLMAUX */
...
#ifdef CLMAUX
/*  exclude code overriding AUX (RDR: and PUN:) ports */
#else	/* CLMAUX */
	pid_rec = fork();
	switch (pid_rec) {
	case -1:
...
/*
 *	I/O handler for read auxs_in status:
 *	0xff : input available
 *	0x00 : no input available
 */
static BYTE auxs_in(void)
{
	...
	if (++busy_loop_cnt_aux >= MAX_BUSY_COUNT) {
		SLEEP_MS(1);
		busy_loop_cnt_aux = 0;
	}

	if (aux_in_eof)
	  return ((BYTE) 0xF0);	/* tell program that AUX has errors has stopped talking */
...
#ifdef CLMAUX
/*
 *	I/O handler for read auxin data
 */
static BYTE auxd_in(void)
{
	...
	FD_ZERO(&rset);
	FD_SET(auxin, &rset);
	select(auxin+1, &rset, NULL, NULL, NULL);
	if (FD_ISSET(auxin, &rset)) {
	  if ( (n = read(auxin, &c, 1)) < 0) {
...
#ifdef CLMAUX
/*
 *	I/O handler for write auxout data:
 *	the output is written to the terminal
 */
...
again:
	if (write(auxout, (char *) &data, 1) != 1) {
		if (errno == EINTR) {
			...
			LOGE(TAG, "can't write auxiliary console");
			//cpu_error = IOERROR;
			//cpu_state = STOPPED;
			aux_in_eof = 0x08;
		}
